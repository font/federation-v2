...
# Globals
GITHUB_REMOTE_FORK_NAME="origin"
GITHUB_REMOTE_UPSTREAM_NAME="upstream"
GITHUB_PR_BASE_BRANCH="kubernetes-sigs/master"
...
function create-release-pr() {
  ...
  git checkout -b "${RELEASE_TAG}-rel" "${GITHUB_REMOTE_UPSTREAM_NAME}/master"
  ...
  hub pull-request --base "${GITHUB_PR_BASE_BRANCH}" -m "Update repo for release ${RELEASE_TAG}"
}
...
util::log "Updating master branch with release changes"
create-release-pr
